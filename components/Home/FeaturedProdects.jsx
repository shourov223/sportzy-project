...
export const ProductCard = ({ image, title, price, category, id }) => {
  return (
    <Link
      href={`/details/${id}`}
      className="border border-gray-200 p-3 sm:p-4 rounded-xl sm:rounded-2xl overflow-hidden group hover:shadow-lg transition-shadow duration-300 bg-white flex flex-col"
    >
      ...
      <div className="mt-3 flex flex-col gap-1 flex-1">
        <p
          className={`text-[10px] sm:text-xs font-semibold text-red-500 uppercase tracking-wide ${montserrat.className}`}
        >
          {category}
        </p>
        <h3
          className={`text-sm sm:text-base md:text-lg font-bold text-black line-clamp-2 leading-snug flex-1 ${jost.className}`}
        >
          {title}
        </h3>
        <p
          className={`text-sm sm:text-base md:text-lg text-black font-semibold mt-1 ${montserrat.className}`}
        >
          ${price}
        </p>
      </div>
    ...
    <section className="py-8 sm:py-10 md:py-12 lg:py-16 bg-gray-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-5 sm:pb-6 md:pb-7 border-b border-gray-300 gap-3 sm:gap-4">
          <h2
            className={`text-black font-bold text-xl sm:text-2xl md:text-3xl lg:text-[36px] leading-tight ${jost.className}`}
          >
            Featured Products
          </h2>
          ...
          <div className="hidden lg:flex items-center gap-1 flex-wrap justify-end">
            {categories.map((category, index) => (
              <button
                ...
                className={`px-3 xl:px-4 py-1.5 transition-all duration-200 text-sm xl:text-base capitalize whitespace-nowrap ${
        ...
        <div className="pt-5 sm:pt-6 md:pt-8 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
          {filteredProducts.length > 0 ? (
            ...
            <div className="col-span-full text-center py-12 md:py-16">
              <p
                className={`text-gray-500 text-base sm:text-lg ${montserrat.className}`}
              >
